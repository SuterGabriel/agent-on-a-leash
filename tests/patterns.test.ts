...
describe("lookalike: an identical normalised name is the same brand", () => {
  ...
  it('"Night Owl Kitchen" vs established "NightOwl Kitchen": same brand, not an imitation', () => {
    const r = run(I, event(I, at("ME_OWL2", "Night Owl Kitchen", "food_delivery")), new Ledger(), base);
    expect(r.reason_codes).not.toContain("lookalike_shop");
  });
  it('"PixelHarbour" vs established "PixelHarbor": an imitation', () => {
