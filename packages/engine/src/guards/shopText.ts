...
// is a reason to ASK, or to DECLINE once the customer has learned the rule "always decline when a shop's text
// gives orders". It can never make a purchase pass; any other decline still wins.
import type { Guard } from "../types";

export const shopTextManipulation: Guard = ({ shop, policy }) => {
  if (shop.flagged.length === 0) return { guard: "shop_text", verdict: "PASS", evidence: [] };
  const first = shop.flagged[0];
  return {
    guard: "shop_text",
    verdict: policy.shopTextAction === "decline" ? "DECLINE" : "STEP_UP",
