...
| A2 | Jury figures from the live server | **done** | `npm run jury-figures -- --answered approve\|decline --out <name> <run ids>` writes `docs/pitch/<name>.md` (aggregates, committed) and `reports/<name>-full.md` (every purchase, git-ignored). Two passes on 25 Sep 00:00: asks answered yes, 7 scenarios, 74 purchases, 38 % asked, 0 missed, 268 ms median; asks answered no, 10 scenarios, 111 purchases, 59 % asked, 0 missed, 327 ms median | Quote the yes pass; keep the no pass as the honest floor | `packages/backend/src/cli/juryFigures.ts`, `docs/learnings/2026-09-25-live-before-after.md` |
...
| Live, asks answered yes (25 Sep 00:00) | 74 purchases · 20 approve · 28 step_up · 26 decline · 0 missed · 268 ms median | `docs/pitch/live-figures-asks-approved.md` |

Relay quotes 28.9 percent asked on the public set, where every card has history. Our live cards have none, so every first purchase at a shop asks once; 38 percent on the live set is the number to defend, and the reason is on the slide.
